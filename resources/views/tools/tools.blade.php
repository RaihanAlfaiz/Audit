...
<span class="text-muted fw-light">Tools / Checklist Event</span>
@endsection
@section('content')
<div class="card">
    <div class="card-header">
        <h3>Checklist Alat untuk {{ $event->tenant_name }}</h3>
    </div>
    <div class="card-body">
        @if (session('status'))
        ...
        <form id="checklistForm" action="{{ route('tools.submit', $event->id) }}" method="POST">
            @csrf
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function toggleSubmitButton() {
            ...
            statusInput.value = allChecked ? 'ready' : 'pending';
        }
